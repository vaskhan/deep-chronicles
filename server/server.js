...
import { calcDmg, missChance, evaChance, flatDist, clamp, mobAtk, mobCrit, MOB_SPREAD } from '../src/sim.js';
...
wss.on('connection', (ws, req) => {
  ...
  players.set(p.id, p);
  send(p, { t: 'hi', online: online(), features: { groundLoot: 1, progression: 1, autoloot: 1, crafting: 1, nativeOnly: 1, heartbeat: 1, combatTelegraphs: 1, party: 1, timedEffects: 1, eliteMobs: 1, mobPacks: 1 } });
  ws.on('message', (raw) => {
...
setInterval(() => {
  ...
  for (const p of players.values()) {
    ...
    for (const q of list) {
      ...
      const row = [q.id, +q.x.toFixed(2), +q.y.toFixed(2), +q.z.toFixed(2), +q.r.toFixed(2), q.anim | 0, Math.round((q.P.hp / PL.statsOf(q, now).maxHp) * 100), status(q)];
