...
func snapshot(row: Array, timestamp: float):
	...
	# Последний столбец снапшота — активные эффекты; сервер шлёт его только когда они есть.
	var fx_column = 7 if kind == "m" else 8
	effects = row[fx_column] if row.size() > fx_column else []
