...
func _run():
 ...
 for shop in data.world.townShops:
  ...
  var npc=load("res://scripts/actor.gd").new();npc.kind="n";root.add_child(npc);npc.setup("merchant",shop.name)
  npc.position=data.position_at(shop.x+3.9*shop.modelScale,shop.z-7*shop.scale);npc.rotation.y=-PI/2
