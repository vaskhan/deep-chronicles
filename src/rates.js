...
export function rollDropsRated(mobDef, rates = DEFAULT_RATES, rng = Math.random, factor = 1) {
  const out = [];
  // dropMul — ранговый множитель элиты и чемпиона (src/elites.js). Он вшит в определение
  // моба при спавне и применяется один раз, до рейтов и до штрафа за разницу уровней.
  const rankMul = mobDef.dropMul || 1;
  for (const [id, chance] of Object.entries(mobDef.drops || {})) {
    if (rng() >= rateDropChance(Math.min(1, chance * rankMul), rates, factor)) continue;
