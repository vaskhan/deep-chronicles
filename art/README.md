...
23.09.2026, второй проход маршрута: те же CC0 `moss_boulder.glb`, `fern_02.glb` и карты Poly Haven переиспользуются для береговых групп, низких мшистых кочек, пятен гравия и материала заставы. Новых внешних ассетов нет. Мох и намокание камня, вода, пена, круги и пыль — локальные шейдеры/геометрия; исходные UV и закреплённые файлы не изменены. Группы папоротника нормализованы до 0.35–0.85 м. Крупные камни размещены у стен, в русле — низкий декоративный камень без новых коллизий.

## Противники Громового ущелья — 23.09.2026

Три модели маршрута заменены отдельными GLB с каноническим скелетом и общей библиотекой клипов:

| Противник | Основа и дополнения | Треугольники |
|---|---|---:|
| Воин племени Клыка | Orc из Quaternius Ultimate Monsters; меховая накидка/набедренник, дубина перенесена в правую руку | 7472 |
| Шаман племени Клыка | Tribal из того же пака; высокий убор, отдельный рогатый посох | 6748 |
| Каменный страж | Сочленённое тело из Rock Moss Set 01 / Poly Haven, глаза, каменная защитная плита | 10565 |

[Quaternius Ultimate Monsters](https://quaternius.com/packs/ultimatemonsters.html) — **CC0-1.0** (проверено на странице автора). Оригиналы `Orc.gltf`, `Tribal.gltf` и встроенные авторские атласы закреплены в `art/sources/gorge-mobs/`. Исходный `License.txt` сохранён без изменений в `art/licenses/quaternius-ultimate-monsters.txt`: автор оставил в нём заголовок Ultimate Platformer Pack, поэтому CC0 дополнительно сверена с отдельным полем License на странице Ultimate Monsters. Новый Bestiary не использован: его QAL не соответствует требованию CC0.

[Rock Moss Set 01](https://polyhaven.com/a/rock_moss_set_01) — **CC0-1.0**, [условия автора](https://polyhaven.com/license). Это уже закреплённый скан `godot/assets/gorge/moss_boulder.glb`; полные авторы и исходные URL — в реестре ущелья. Сетка одного камня сокращена и собрана в жёсткие части, привязанные к каноническим костям; исходные UV и PBR-карты сохранены. Дополнительные формы меха, посоха и стража, а также текстура шкуры создаются локальным скриптом и также переданы под CC0 (код скрипта сохраняет лицензию проекта).

У старого Ultimate Monsters другой скелет: исходная T-поза подогнана по суставам, веса переименованы в канонические группы; заимствованные анимации пака не используются. Дубина изначально в левой руке, подготовка переносит её в правую под Sword_Attack. Материалы шероховатые, атласы приглушены, карты до 1024 px, mipmaps включены. Модели остаются стилизованными, а не фотореалистичными персонажами.

Воспроизведение: `GLTF_TRANSFORM_BIN=/путь/glTF-Transform-4.5.0/gltf-transform node tools/godot/prepare-gorge-mobs.mjs` (Blender в PATH либо `BLENDER_BIN`), затем `npm run native:assets` и `npm run native:mob-models`. Нет обращения к GPU/OCR или генеративным сервисам. Источники, лицензии и SHA-256 — `art/sources/gorge-mobs/sources.json`; хеши готовых файлов/число треугольников — `godot/assets/gorge-mobs/manifest.json`.

## Сдержанная стилизация — второй проход моделей, 23.09.2026

Контракт — `docs/ART_STYLE.md`. Клык сохраняет основу Quaternius Ultimate Monsters CC0:
гребень/высокие перья удалены, добавлены тёмные косы с костяными обоймами, у шамана
костяной убор. Глаза уменьшены, кожа серо-оливковая с мелкой фактурой, у воина мех и
ржаво-коричневый пояс, у шамана тёмный петроль. Процедурные карты кожи/меха/ремней
создаются локально, без внешнего GPU. UV палитры заменены развёрткой под фактуру.

| Модель | Треугольники | Источник |
|---|---:|---|
| Воин Клыка | 7184 | Quaternius Orc + авторские CC0 дополнения |
| Шаман Клыка | 6036 | Quaternius Tribal + авторские CC0 дополнения |
| Скальный паук | 2902 | Poly Haven Rock Moss Set 01 + авторские CC0 ноги, жвалы, риг и 8 клипов |
| Страж заставы | 6204 | KayKit Skeleton_Warrior + авторский CC0 башенный щит, меч и элементы брони |
| Каменный страж | 10565 | Предыдущая модель без художественных изменений |

**KayKit Character Pack: Skeletons 1.0**, Kay Lousberg — именно **CC0-1.0**.
Проверены README и LICENSE.txt официального репозитория
<https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Skeletons-1.0>,
фиксированный commit `15b62b9bad122f72926c10fb14d622c73819fa54`.
Оригинал лицензии: `art/licenses/kaykit-skeletons.txt`; исходный GLB и SHA-256:
`art/sources/gorge-mobs/`. EXTRA/SOURCE-пакеты не используются. Старый скелет KayKit
подогнан в исходной позе под канонический риг; общие анимации — Quaternius CC0.

Паук не использует прежнюю TRELLIS-модель: его панцирь — настоящий CC0-скан,
сегментированные ноги и анимации построены локальным скриптом. Сканы сохраняют
свою текстуру; ноги имеют отдельную бесшовную фактуру, чтобы не растягивать атлас.
Все новые дополнения и текстуры противников переданы под CC0, что закреплено в
`sources.json`; лицензия кода инструментов остаётся лицензией проекта.

Герои: приглушены насыщенность ткани воина и мантии мага, поднята шероховатость
неметаллических участков. Тяжёлый вариант воина уже подходит и не изменён.
`prepare-hero-palettes.mjs` меняет только картинки внутри GLB и вынесенные `.webp`;
геометрия, узлы, веса, риг и клипы сохраняются. Исходные карты и хеши —
`art/sources/hero-palettes/`. Это существующие текстуры проекта с прежним происхождением,
описанным выше, а не новый сторонний CC0-пак; их лицензия этим проходом не переименована.
