...
function simulate(rates=DEFAULT_RATES) {
  ...
  for(let lvl=1;lvl<40;lvl++) {
    ...
    if([3,5,8,12,18,25,32,36,40].includes(lvl+1)) checkpoints.push({level:lvl+1,blocked:Boolean(blockedAt),kills,coins:Math.round(coins),optionalSales:Math.round(sales),sp,learn:Object.fromEntries(Object.entries(CLASSES).map(([id,c])=>[id,c.skills.flatMap(skillRanks).filter(s=>s.lvl<=lvl+1).reduce((n,s)=>n+s.sp,0)]))});
  }
...
// Громовое ущелье: убийства от 25 до 32/36/40 и состав ярусов — из тех же чекпоинтов и реальной расстановки.
const at=level=>checkpoints.find(r=>r.level===level);
const since25=level=>at(level)&&!at(level).blocked?money(at(level).kills-at(25).kills):'—';
const gorgeKinds=[...new Set(spawns.filter(sp=>sp.pack).map(sp=>sp.mob))].sort((a,b)=>MOBS[a].lvl-MOBS[b].lvl);
const gorgeNote=`Громовое ущелье (src/gorge.js): стай — ${new Set(spawns.filter(sp=>sp.pack).map(sp=>sp.pack)).size}, мобов в них — ${spawns.filter(sp=>sp.pack).length}, видов 25–39 уровня — ${gorgeKinds.length}. Опыт продолжает кривую обычных мобов (≈ 2.95·L^1.8), монеты ≈ 0.14 опыта. Убийств от 25 уровня: до 32 — ${since25(32)}, до 36 — ${since25(36)}, до 40 — ${since25(40)}.`;
const gorgeTable=['| Вид | Уровень | Жизнь | Опыт | Монеты | Ресурсы и вещи |','|---|---:|---:|---:|---:|---|',...gorgeKinds.map(k=>{const m=MOBS[k];return `| ${m.name} | ${m.lvl} | ${money(m.hp)} | ${money(m.xp)} | ${m.coins[0]}–${m.coins[1]} | ${Object.entries(m.drops).map(([id,p])=>`${ITEMS[id].name} ${+(p*100).toFixed(1)} %`).join(', ')} |`;})];
...
'## Громовое ущелье и предел контента','',gorgeNote,'',...gorgeTable,'',`Ступенчатый штраф за разницу уровней делает мобов бесполезными с разницы в десять уровней, поэтому прокачка упирается в последний уровень, для которого есть мобы. ${checkpoints.blockedAt?`Сейчас это ${checkpoints.blockedAt+1} уровень — дальше в таблице прочерк, а не выдуманные числа.`:'С Громовым ущельем (мобы 25–39) все уровни до 40 проходятся обычными мобами без штрафа: для каждого уровня есть моб не старше героя и не младше на три.'} Основной законченный цикл снаряжения — D → C → B на уровнях 8 / 18 / 25; в ущелье C-вещи выпадают на террасах и у водопада, B-части — редко (0.4–0.5 % за штуку) на вершине, B-оружие по-прежнему только через печати босса. Массовая торговля, кланы, рынок ресурсов и инфляция между игроками ещё не реализованы. Не обещать им готовый баланс.`,''];
...
if(process.argv.includes('--compare')) {
  ...
  const table=['','## Влияние рейтов на путь до 25 и 40 уровня','','Равномерные множители опыта/SP/монет/шанса дропа. Остальные коэффициенты ×1. Прочерк — уровень недостижим: с разницы в десять уровней награды нет, а мобов выше 39 уровня нет.','',
